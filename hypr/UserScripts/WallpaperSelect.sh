...
# Sorting Wallpapers by filename
menu() {
   sorted_full=($(for pic in "${PICS[@]}"; do printf "%s %s\n" "$(basename "$pic")" "$pic"; done | sort | cut -d' ' -f2- ))

   printf "%s\x00icon\x1f%s\n" "$RANDOM_PIC_NAME" "$RANDOM_PIC"

   for pic_path in "${sorted_full[@]}"; do
     pic_name=$(basename "$pic_path")
     if [[ "$pic_name" =~ \.gif$ ]]; then
       rel_path="${pic_path#$wallDIR/}"
       cache_gif_image="$HOME/.cache/gif_preview/${rel_path//[\/]/_}.png"  # replace / with _ for cache name
       ...
       rel_path="${pic_path#$wallDIR/}"
       cache_preview_image="$HOME/.cache/video_preview/${rel_path//[\/]/_}.png"
...
modify_startup_config() {
  ...
  # Check if it's a live wallpaper (video or gif)
  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm|gif|GIF)$ ]]; then
...
apply_video_wallpaper() {
  ...
  # Apply video wallpaper to each monitor using mpvpaper
...
main() {
  ...
  if [[ -z "$choice" ]]; then
   ...
   # Construct the selected file path
   if [[ "$choice" == "$RANDOM_PIC_NAME" ]]; then
     selected_file="$RANDOM_PIC"
   else
     selected_file=$(find "$wallDIR" -name "$choice" -type f | head -1)
   fi
  ...
  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm|gif|MP4|MKV|MOV|WEBM|GIF)$ ]]; then
